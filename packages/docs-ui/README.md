...
| `AIChat.vue` | AI 对话 UI | `send` `ready` `removeContext` `error` | `useAIChat` |
...
| `useAIChat.ts` | `useAIChat` | 对话流式发送、上下文窗口管理、终止/清空、token 估算 |
...
- 类型：`ChatMessage` `ChatRequest` `ChatStreamEvent` `ContextConfig`
...
- `src/components/index.ts`：导出 `SmartTree` `AIChat` `DocumentParsedWorkspace` `DocBlocksTreeNode`
...
| `AIChat.vue` | `useAIChat`（B） | `send` `ready` `removeContext` `error` | A 层业务组件，状态在 B |
...
| `useAIChat.ts` | 对话参数与上下文配置 | `messages/loading/sendMessage/stopGeneration` | 是（`/api/chat`） |
