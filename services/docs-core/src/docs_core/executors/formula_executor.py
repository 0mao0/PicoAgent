...
from docs_core.retrieval.formula_retriever import formula_retriever
...
class FormulaExecutor:
    ...
    def execute(
        ...
        formula_candidates = formula_retriever.retrieve(request, doc_nodes)
        ranked_candidates: List[RetrievedItem] = formula_candidates[: max(1, min(20, request.top_k))]
        ...
        fusion_debug = {
            "sources": {
                "formula_aware": {
                    "input_hits": len(formula_candidates),
                    "weight": 1.0,
                }
            },
            "deduped_hits": len(formula_candidates),
